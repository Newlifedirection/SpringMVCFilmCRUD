...
<body>
	...
	<c:choose>
		<c:when test="${! empty film}">
			<div class="alert alert-primary" role="alert">Update Success!</div>
			<h2>Film Details</h2>
			<ul>
...
<body>
	...
	<c:choose>
		<c:when test="${! empty film}">
			<h2>Film ID ${film.filmId} Details</h2>
			<ul>
				<li><strong>Film ID:</strong> ${film.filmId}</li>
				<li><strong>Title:</strong> ${film.title}</li>
				<li><strong>Description:</strong> ${film.description}</li>
		...
		<c:otherwise>
			<div class="alert alert-primary" role="alert">Film Not Found! Did you delete something?</div>
		</c:otherwise>
	</c:choose>
	<%-- 	<form action="GetFilmData.do" method="GET">
